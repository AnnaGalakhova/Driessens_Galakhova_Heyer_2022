function [p, tbl, comp, NMIQR, X] = dotplot_genes(Data, cpm, genes, genevar, ylimit)
    ...
    dat = Data(ismember(Data.NewLabel,newlabel),{'sample_id','NewLabel'}) ;
   ...
   % select = standardizeMissing(select,0,'DataVariables',2:width(select)) ; % Optional, But probably necessary. Changes all zeros to NaN
    ...
    switch genevar
        case 'AvgExpLog10'
            select.genescore = mean(log10(select{:,2:width(select)}+1),2, 'omitnan') ;
        case 'MedExp'
            select.genescore = median(select{:,2:width(select)},2,'omitnan') ; 
        case 'AvgExp'
            select.genescore = mean(select{:,2:width(select)},2, 'omitnan') ;
        case 'SumExpLog10'
