function [p, tbl, comp, NMIQR, X] = dotplot_selected_genes(Data, cpm, genes, ylimit)
   ...
   % select = standardizeMissing(select,0,'DataVariables',2:width(select)) ; % Optional, But probably necessary. Changes all zeros to NaN
    ...
    realfig = figure('Position', [2000 10 1100 300]) ;
    ...
    box off
    ylabel('Mean gene expression selected genes log10(cpm+1)');
    ...
    comp1 = multcompare(stats, 'CriticalValueType','tukey-kramer') ;
