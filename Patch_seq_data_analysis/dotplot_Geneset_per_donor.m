function [p, tbl, comp, NMIQR, X] = dotplot_Geneset_per_donor(Data, cpm, genes, genevar, ylimit)
    ...
    dat = Data(ismember(Data.NewLabel,newlabel),{'sample_id','NewLabel','donor_label'}) ;
    dat = sortrows(dat,'NewLabel') ;
    
   

    genes = genes(ismember(genes,cpm.Properties.VariableNames)) ;
    select = cpm(ismember(cpm.sample_id,dat.sample_id),['sample_id'; genes]) ;
   % select = standardizeMissing(select,0,'DataVariables',2:width(select)) ; % Optional, But probably necessary. Changes all zeros to NaN
    
    
    switch genevar
        case 'AvgExpLog10'
            select.genescore = nanmean(log10(select{:,2:width(select)}+1),2) ;
        case 'MedExp'
            select.genescore = median(select{:,2:width(select)},2,'omitnan') ; 
        case 'AvgExp'
            select.genescore = nanmean(select{:,2:width(select)},2) ;
        case 'SumExpLog10'
            select.genescore = sum(log10(select{:,2:width(select)}+1),2,'omitnan') ;
        case 'SumExp'
            select.genescore = sum(select{:,2:width(select)},2,'omitnan') ; 
        otherwise
            error(['< ' genevar ' > is not a valid gene aggregate score. please respecify'])
    end
    dat=dat(ismember(dat.sample_id, cpm.sample_id),:);
    dat = join(dat,select,'Keys','sample_id') ;
    
    % select and prepare data
    dat2 = table2array(dat(:,{'genescore'})) ;
    ...
    for i=1:nclust
        ...
        test=dat2(T==i);
        donor=[];
        donor=findgroups(dat.donor_label(dat.NewLabel==i));
        max_size=unique(donor);
        X(1:size(max_size,1),i)=splitapply(@mean,test,donor);
    ...
    realfig = figure('Position', [2000 10 1100 400]) ;
    set(findall(gcf,'-property','FontSize'),'FontSize',12)
    set(findall(gcf,'-property','FontName'),'FontName','Arial')
    
    subplot(1,2,1);

    dotdensity(X, 'medianLine', 'on', 'labels', labels, 'dotEdgeColor', [0.2 1 0.2; 0.7 0.7 0.7; 0.5 0.8 1; 0.1 0.8 0.6; 0 0 1; 0.8 0.2 0.8], 'dotFaceColor', 'w','dotSize',5, 'lineColor','k' , 'lineWidth', 1.5, 'spread', 0.5);        
    ...
    %savename = [genevar '_of_' gset.geneset '_' WhichCells] ;
